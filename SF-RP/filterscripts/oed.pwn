    ...
    if (IsPlayerAdmin(playerid)) {
        ...
        if (strcmp(cmd, "/oadd", true) == 0) {
            ...
            for (new i=1;i<=gObjectCount;i++) {
                if (strlen(gObjects[i][Name]) != 0 && strcmp(gObjects[i][Name], name, true) == 0){
                    SystemMessage(playerid, "Name Exists!", COLOR_RED);
                    return 1;
                } 
            }
        ...
        //objekt zum bearbeiten auswählen
            ...
            if (oid > 0 && oid <= gObjectCount) {
                ...
                format(msg, sizeof msg, "Object '%s' (ModelID: %d ObjectID: %d) selected", name, gObjects[oid][ModelID], oid);
                SystemMessage(playerid, msg, COLOR_GREEN);
                return 1;
            } else {
        ...
        //deselect a object
            ...
            if(oid)
            {
                format(msg, sizeof msg, "Objekt '%s' (ModelID:%d ObjectID:%d) deselected",  gObjects[oid][Name], gObjects[oid][ModelID], oid);
                gEditingObject[playerid][object_id] = 0;
                gEditingObject[playerid][mode] = OED_NONE;
            }
            else
                format(msg, sizeof msg,"You not have selected a Objekt!");
            SystemMessage(playerid, msg, COLOR_YELLOW);
            return 1;
        }
        ...
        //goto to a objeckt by name
            ...
            new name[MAX_STRING], oid;
            syntax = "SYNTAX: /ogoto [Objektname]";
        ...
        //help command
            ...
            SendClientMessage(playerid,COLOR_GREEN,"/oadd [modelid] [name]: add a object.");
            SendClientMessage(playerid,COLOR_GREEN,"/ocopy [name]: copy a object.");
            SendClientMessage(playerid,COLOR_GREEN,"/odel: delete a selected object.");
            SendClientMessage(playerid,COLOR_GREEN,"/omode [m_xy|m_z|r_xy|r_z]: select mode of a edit object.");
            SendClientMessage(playerid,COLOR_GREEN,"/onext: select a next object arround.");
            SendClientMessage(playerid,COLOR_GREEN,"/osel [Objektname]: select object by name.");
            SendClientMessage(playerid,COLOR_GREEN,"/odesel: deselect a object seleceted.");
            SendClientMessage(playerid,COLOR_GREEN,"/ostick <distance>: stick a object to a player.");
            SendClientMessage(playerid,COLOR_GREEN,"/orelease: destick a object to a player.");
            SendClientMessage(playerid,COLOR_GREEN,"/ofaktor [faktor]: change faktor of a move object.");
            SendClientMessage(playerid,COLOR_GREEN,"/ogoto [Objektname]: goto to a objetct by name.");
            return 1;
        }
        //save all objects in memory
        else if (strcmp(cmd, "/osaveall", true) == 0) {
            SaveObjects();          
            format(msg, sizeof msg, "%d objeket are saves!", gObjectCount);
            SystemMessage(playerid, msg, COLOR_GREEN);
            return 1;
        }        
    }
